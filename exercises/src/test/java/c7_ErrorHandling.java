...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void potato_potato() {
        Mono<String> currentUser = getCurrentUser()
                .onErrorMap( e -> new SecurityException(new Throwable("No active session, user not found!")))
    ...
    public void error_reporter() {
        //todo: feel free to change code as you need
        Flux<String> messages = messageNode().doOnError(e -> errorReportService(e).subscribe());
    ...
    public void unit_of_work() {
        Flux<Task> taskFlux = taskQueue().flatMap(task -> task.execute().then(task.commit()).onErrorResume(e -> task.rollback(e)).thenReturn(task))
    ...
    public void good_old_polling() {
        //todo: change code as you need
        Flux<String> alerts = nodeAlerts().repeatWhenEmpty((comp)-> comp.delayElements(Duration.ofSeconds(1))).repeat();
