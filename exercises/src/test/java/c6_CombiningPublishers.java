...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void i_am_rubber_you_are_glue() {
        ...
        Flux<Integer> numbers = Flux.just(1, 2).flatMapSequential(n -> {
            if (n == 1) {
                return numberService1();
            } else
                return numberService2();
        });
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((m, primary) -> m.get().metaData.equals("spam") ? mailBoxSecondary() : primary);
        mailBoxPrimary();
        mailBoxSecondary();
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds = tasksToExecute().flatMapSequential(task -> task.map(s -> {
            commitTask(s);
            return s;
        }));
...
*/
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(),s -> s,s -> StreamingConnection.closeConnection() );
        //don't change below this line
